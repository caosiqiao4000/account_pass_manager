...
public class CheckHostAsyTask extends AsyncTask<Void, Integer, Boolean> {
	 ...
	 *            // 接口名称
	 */
	public CheckHostAsyTask(Context context,
			IUICallBackInterface iuiCallBackInterface, int caseKey,
			String hostAdd, boolean pdShow, String pdMsg) {
	...
	protected Boolean doInBackground(Void... params) {
		try {
			if (SDKSettings.DEBUG) {// httpHeadStr html =
									// http://user.pay.921.com
				Log.w(SDKSettings.LOG_TAG, "check domain is " + hostAdd);
			}
			InetAddress address;
