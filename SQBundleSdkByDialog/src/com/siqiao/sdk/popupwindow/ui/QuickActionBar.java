...
public class QuickActionBar {
	...
	public QuickActionBar(View anchor, int idx) {
		...
		actionsLayoutAnim = AnimationUtils.loadAnimation(anchor.getContext(), R.anim.anim_actionslayout);
	...
	public void show() {
		...
		qaBarRoot.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
		...
		// 显示/隐藏指定的箭头
		int arrowId = arrowOnTop ? com.siqiao.sdk.dialog.R.id.qa_arrow_up : R.id.qa_arrow_down;
	...
	private void appendActionsItemUI(ViewGroup actionsLayout, Vector<ActionItem> vec) {
		...
		for (int i = 0; i < size; i++) {
			view = getActionItemUI(vecActions.get(i), i);
			// 将本QuickActionBar设为Tag供处理点击事件时隐藏QuickActionBar
			view.setTag(this);
			actionsLayout.addView(view, i);
		}
	}

	private View getActionItemUI(ActionItem item, int index) {
		LinearLayout actionItemLayout = (LinearLayout) inflater
				.inflate(R.layout.action_item_horizontal, null);
		ImageView icon = (ImageView) actionItemLayout.findViewById(R.id.iv_icon);
		TextView txtName = (TextView) actionItemLayout.findViewById(R.id.tv_title);
	...
	public void setPopupWindowBackground(Drawable popupWindowBackground) {
		this.popupWindowBackground = popupWindowBackground;
	}
