...
public class QuickAction extends PopupWindows implements OnDismissListener {
	...
	public QuickAction(Context context, int orientation) {
		...
		mChildPos = 0;
		mColorStyle = COLOR_WHITE;
	}

	public QuickAction(Context context, int orientation, int colorStyle) {
		super(context);
		...
		// >>>black
		if (mColorStyle == COLOR_BLACK) {
			if (mOrientation == HORIZONTAL) {
				setRootViewId(R.layout.popup_horizontal_black);
			} else {
				setRootViewId(R.layout.popup_vertical_black);
			}
		}
		// >>>white
		else {
	...
	public void addActionItem(ActionItem action, boolean isLast) {
		...
		if (mColorStyle == COLOR_BLACK) {
			if (mOrientation == HORIZONTAL) {
				container = mInflater.inflate(R.layout.action_item_vertical_black, null);
			} else {
				container = mInflater.inflate(R.layout.action_item_vertical_black, null);
			}
		} else {
		...
		LinearLayout layout = (LinearLayout) container.findViewById(R.id.action_layout);
		if (!action.isEnable()) {
			layout.setEnabled(false);
			layout.setBackgroundColor(Color.parseColor("#E0EEEE"));
		}
		...
		if (mOrientation == HORIZONTAL) {
			container.findViewById(R.id.ImageViewHeadLine).setVisibility(View.GONE);
		}

		if (isLast && mOrientation != HORIZONTAL) {
			container.findViewById(R.id.ImageViewHeadLine).setVisibility(View.GONE);
		}
	...
	public void setActionItems(List<ActionItem> actionItems) {
		...
		for (int i = 0; i < actionItems.size(); i++) {
			if (i == actionItems.size() - 1) {
				addActionItem(actionItems.get(i), true);
			} else {
				addActionItem(actionItems.get(i));
			}
		}
	}
	...
	public void show(View anchor) {
		...
		if (onTop) {
			...
			yPos = anchorRect.bottom - anchor.getHeight() / 2;
		...
		int padingh = showArrow(((onTop) ? R.id.arrow_down : R.id.arrow_up), arrowPos);

		setAnimationStyle(screenWidth, anchorRect.centerX(), onTop);
		if (mArrowUp.getVisibility() == View.VISIBLE) {
			mWindow.showAtLocation(anchor, Gravity.NO_GRAVITY, xPos, yPos + padingh);
		} else {
			mWindow.showAtLocation(anchor, Gravity.NO_GRAVITY, xPos, yPos + padingh);
		}
	}
