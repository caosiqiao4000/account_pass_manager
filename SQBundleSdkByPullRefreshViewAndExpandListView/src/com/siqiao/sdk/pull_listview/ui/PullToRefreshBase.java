...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private String tag;// 更新时间，保存在本地数据库，SP以类名来算
...
//	private int mMode = MODE_PULL_DOWN_TO_REFRESH;
	...
	protected void resetHeader() {
		...
		if (null != mHeaderLayout) {
			mHeaderLayout.reset(tag);
		}
		if (null != mFooterLayout) {
			mFooterLayout.reset(tag);
		}
	...
	public void setRefreshingInternal(boolean doScroll) {
		...
		if (null != mHeaderLayout) {
			mHeaderLayout.refreshing(tag);
		}
		if (null != mFooterLayout) {
			mFooterLayout.refreshing(tag);
		}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
			mMode = a.getInteger(R.styleable.PullToRefresh_ptrMode,
					MODE_PULL_DOWN_TO_REFRESH);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrTag)) {
			tag = a.getString(R.styleable.PullToRefresh_ptrTag);
		}
		...
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
			mFooterLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH,
					releaseLabel, pullLabel, refreshingLabel, a, getTag());
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
			mRefreshableView.setBackgroundResource(a.getResourceId(
					R.styleable.PullToRefresh_ptrAdapterViewBackground,
					Color.WHITE));
		}
		a.recycle();
		a = null;
